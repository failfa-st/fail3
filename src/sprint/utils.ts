 ...
 * @returns {string} The prompt for creating a Cypress test.
 */
export function buildServerlessFunctionPrompt(dataModel: string) {
 ...
 *   test.
 */
export async function prepareServerlessFunction(dataModel: string, ai: AI) {
	const task = buildServerlessFunctionPrompt(dataModel);
	return sendRequest(task, ai);
}
...
export async function createServerlessFunction(
	...
	const task = await prepareServerlessFunction(dataModelFile.content, ai);
